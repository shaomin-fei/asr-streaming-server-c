...
std::shared_ptr<FeaturePipeline> ConnectionHandler::GetCurrentPipeline() {
    
    if (!b_passWholeSentenceFromFrontend) {
        return feature_pipeline_;
    }
    std::unique_lock<std::mutex> lock(*mutex_.get());
    if (feature_pipelines.size() == 0) {
        return nullptr;
    }
    return feature_pipelines[0];
}
...
void ConnectionHandler::OnSpeechEnd() {
  ...
  std::shared_ptr<FeaturePipeline> pipeline = GetCurrentPipeline();
  if (pipeline != nullptr) {
...
void ConnectionHandler::OnSpeechData(const beast::flat_buffer& buffer) {
  ...
  std::shared_ptr<FeaturePipeline>pipe= std::make_shared<FeaturePipeline>(*feature_config_);
  pipe->AcceptWaveform(pcm_data, num_samples);
  std::unique_lock<std::mutex> lock(*mutex_.get());
  feature_pipelines.push_back(pipe);
  if (!decoder_) 
  {
      decoder_ = std::make_shared<AsrDecoder>(pipe, decode_resource_,
          *decode_config_);
  }
  if (!decode_thread_) {
  ...
  try {
    while (true) {
      ...
      if (state == DecodeState::kEndFeats) {
        ...
        // TODO,use this message in the front end to determine if the current sentence decode is completed 
        OnFinish();
        //stop_recognition_ = true;
        // activate next featurepipe
        {
            std::unique_lock<std::mutex> lock(*mutex_.get());
            feature_pipelines.erase(feature_pipelines.begin());
        }
        ...
        while (!current) {
            ...
            current = GetCurrentPipeline();
        }
        decoder_->SetFeaturePipeline(current);
        //break;
